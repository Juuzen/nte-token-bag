...
const INITIAL_BAG: Bag = { positive: 0, negative: 0, random: 0 };
...
type InternalAction =
  | { type: "_addPlayer"; player: Player }
  | (ClientMsg & { _myId?: string; _myName?: string });
...
function performDraw(
  state: RoomState,
  by: string
): { nextState: RoomState; token: DrawnToken } | null {
  ...
  const nextBag: Bag = { ...bag, [drawn]: bag[drawn] - 1 };
...
function reducer(state: RoomState, action: InternalAction): RoomState {
  switch (action.type) {
    case "_addPlayer": {
    ...
    case "requestDraw": {
      ...
      const alreadyPending = state.pendingRequests.some(
...
export function useRoom(params: {
  ...
  function send(msg: ClientMsg) {
    dispatch(msg as InternalAction);
  }
